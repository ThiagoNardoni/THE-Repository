...
// Apaga várias despesas de uma vez, dado um array de ids (usado para excluir só os
// lançamentos filtrados, em lotes de 200 para não estourar o limite da requisição)
export async function deleteDespesasPorIds(ids) {
  const TAMANHO_LOTE = 200
  for (let i = 0; i < ids.length; i += TAMANHO_LOTE) {
    const lote = ids.slice(i, i + TAMANHO_LOTE)
    const { error } = await supabase.from('despesas').delete().in('id', lote)
    if (error) throw error
  }
}
